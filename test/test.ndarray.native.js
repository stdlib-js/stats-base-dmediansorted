...
var dmediansorted = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
	'skip': ( dmediansorted instanceof Error )
};
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof dmediansorted, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 4', opts, function test( t ) {
	t.strictEqual( dmediansorted.length, 4, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the median value of a sorted strided array', opts, function test( t ) {
	var x;
	var v;

	x = new Float64Array( [ -3.0, -2.0, -1.0, 1.0, 2.0, 3.0 ] );
	v = dmediansorted( x.length, x, 1, 0 );
	t.strictEqual( v, 0.0, 'returns expected value' );

	x = new Float64Array( [ 3.0, 2.0, 1.0, 0.0, -1.0, -2.0, -3.0 ] );
	v = dmediansorted( x.length, x, 1, 0 );
	t.strictEqual( v, 0.0, 'returns expected value' );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = dmediansorted( 0, x, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = dmediansorted( -1, x, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'if provided a `stride` parameter equal to `0`, the function returns the first indexed element', opts, function test( t ) {
	...
	v = dmediansorted( x.length, x, 0, 0 );
	t.strictEqual( v, 1.0, 'returns expected value' );
